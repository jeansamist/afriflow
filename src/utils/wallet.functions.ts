...
function computeIncludedMinutes(config: ProPlanConfig, isPremium: boolean): number {
  return config.includedMinutes + (isPremium ? config.premiumBonusMinutes : 0);
}

async function fireAndForgetEmail(args: Parameters<typeof import("@/lib/email/send.server").sendAppEmail>[0]) {
  try {
...
export const getPhoneWallet = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    if (!wallet) {
      const trial_ends = new Date(Date.now() + TRIAL_DAYS * 24 * 3600 * 1000).toISOString();
      ...
      if (wallet) {
        await admin.from("minute_transactions").insert({
          ...
          kind: "TRIAL_GRANT",
          bucket: "INCLUDED",
          minutes_delta: TRIAL_MINUTES,
          reference: "Essai découverte 7 jours",
        });
      }
    }
...
export const purchaseTopUp = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: w } = await admin
      .from("phone_wallets")
      .select("extra_seconds, plan_status")
      .eq("user_id", userId)
      .single();
    if (!w) throw new Error("Wallet introuvable");
    ...
    if (email) {
      const { data: prof } = await supabase.from("profiles").select("first_name").eq("id", userId).maybeSingle();
...
export const subscribePro = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const cycleEnd = new Date(Date.now() + PRO_PLAN.cycleDays * 24 * 3600 * 1000).toISOString();
    const { error } = await admin
      .from("phone_wallets")
      .update({
        plan_status: "ACTIVE",
        plan_name: PRO_PLAN.name,
        included_minutes: PRO_PLAN.includedMinutes,
        included_used_seconds: 0,
    ...
    await admin.from("minute_transactions").insert({
      ...
      kind: "PLAN_SUBSCRIBE",
      bucket: "INCLUDED",
      minutes_delta: PRO_PLAN.includedMinutes,
      reference: `PRO-${Date.now().toString(36).toUpperCase()}`,
      metadata: { plan: "PRO", cycleDays: PRO_PLAN.cycleDays, simulated: true },
    });
    ...
    if (email) {
      ...
      void fireAndForgetEmail({
        ...
        templateData: { firstName: (prof as any)?.first_name, includedMinutes: PRO_PLAN.includedMinutes, cycleEnd },
      });
    }
    ...
    try {
      ...
      await pushNotification(admin, {
        ...
        body: `${PRO_PLAN.includedMinutes} minutes incluses · cycle de ${PRO_PLAN.cycleDays} jours.`,
        linkTo: "/dashboard",
      });
    } catch (e) { console.error("[notifications] pro", e); }
...
type ProfileInitiateRow = { kyc_status: string; first_name: string | null; last_name: string | null };
...
export const getProPlanOffer = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [{ data: profileData }, { data: allocData }] = await Promise.all([
      supabase.from("profiles").select("is_premium" as "id").eq("id", userId).maybeSingle(),
      supabase
    ...
    return {
      ...
      premiumBonusMinutes: config.premiumBonusMinutes,
...
export const initiateProSubscription = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: allocData } = await supabase
      ...
      .maybeSingle();
    if (!allocData)
      throw new Error("Aucun numéro de téléphone trouvé. Finalisez d'abord le choix de votre numéro.");
...
export const finalizeProSubscription = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [{ data: profileData }, { data: allocData }] = await Promise.all([
      supabase.from("profiles").select("is_premium, first_name" as "id").eq("id", userId).maybeSingle(),
      admin
        .from("phone_allocations")
    ...
    const allocation = allocData as AllocationRow | null;
    ...
    return { ok: true, alreadyActivated: false, includedMinutes, cycleEnd };
  });
