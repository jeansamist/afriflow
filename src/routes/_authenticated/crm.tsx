...
function CrmPage() {
  ...
  const [prefill, setPrefill] = useState<{ contact_phone?: string; contact_name?: string } | null>(
    null,
  );
  ...
  useEffect(() => {
    if (search.newContactPhone) {
      setPrefill({
        contact_phone: search.newContactPhone,
        contact_name: search.newContactName ?? "",
      });
      setEditing(null);
      setOpen(true);
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 sm:px-6 py-3 sm:py-4">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <ClientDialog
                key={`${editing?.id ?? "new"}-${prefill?.contact_phone ?? ""}`}
                client={editing}
                prefill={prefill}
      ...
      <main className="mx-auto max-w-7xl px-4 sm:px-6 py-6 sm:py-10">
        ...
        <p className="text-sm text-muted-foreground">
          Vos clients à l'étranger : appelez-les et envoyez un lien de paiement en un clic.
        </p>
        ...
        <div className="mt-6 overflow-hidden rounded-2xl border border-border bg-surface">
          {isLoading ? (
            ...
            <div className="p-12 text-center text-muted-foreground">
              ...
              <p className="mt-2 text-sm">
                Créez votre premier client ou chargez des données de démo pour explorer.
              </p>
            </div>
          ) : (
                    ...
                    <TableRow key={c.id}>
                      ...
                      <TableCell className="text-xs text-muted-foreground">
                        {c.lastCall ? (
                          <>
                            <span>
                              {new Date(c.lastCall.created_at).toLocaleDateString("fr-FR")}
                            </span>
                            <span className="ml-2 text-foreground/80">
                              {c.lastCall.duration_seconds}s
                            </span>
                        ...
                        {c.lastPayment ? (
                          <div className="flex flex-col gap-1">
                            <span className="text-sm font-medium">
                              {Number(c.lastPayment.amount).toFixed(2)} {c.lastPayment.currency}
                            </span>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-1">
                          ...
                          <Button
                            ...
                            title="Supprimer"
                            onClick={() => {
                              if (confirm("Supprimer ce client ?")) delMut.mutate(c.id);
                            }}
                          >
          ...
          {clients.length > PAGE_SIZE && (
            <div className="flex items-center justify-between gap-3 border-t border-border p-4 text-xs text-muted-foreground">
              <span>
                Page {page} sur {totalPages} · {clients.length} client(s)
              </span>
      ...
      <Dialog
        open={!!payClient}
        onOpenChange={(o) => {
          if (!o) setPayClient(null);
        }}
      >
        {payClient && (
          <PaymentDialog
            client={payClient}
            payoutCurrency={dash?.counts.payoutCurrency ?? "XOF"}
            onSubmit={(v) => createPayMut.mutate({ ...v, clientId: payClient.id })}
            submitting={createPayMut.isPending}
          />
        )}
      </Dialog>
    </div>
...
function ClientDialog({
  ...
  prefill?: { contact_phone?: string; contact_name?: string } | null;
  onSave: (v: {
    id?: string;
    contact_name: string;
    company_name?: string | null;
    ...
    <DialogContent>
      ...
      <DialogFooter>
        <Button
          ...
          onClick={() => onSave({ id: client?.id, ...form })}
        >
...
function PaymentDialog({
  ...
  const [description, setDescription] = useState(
    `Prestation pour ${client.company_name ?? client.contact_name}`,
  );
  ...
  const { transferFee, serviceFee, net } =
    amt > 0 ? paymentFeeBreakdown(amt, currency) : { transferFee: 0, serviceFee: 0, net: 0 };
  const tooLow = amt > 0 && net <= 0;
  ...
  return (
    <DialogContent className="max-w-lg">
      <DialogHeader>
        ...
        <DialogDescription>
          Le lien sera copié dans votre presse-papier — envoyez-le par email ou WhatsApp.
        </DialogDescription>
      </DialogHeader>
      <div className="grid gap-3">
        <div>
        ...
        {amt > 0 && (
          <div className="mt-2 rounded-xl border border-border bg-surface-elevated p-4 text-sm">
            ...
            <Row label="Frais de transfert" value={`− ${fmt(transferFee, currency)}`} muted />
            <Row
              label={`Frais de service AfriFlow (${(paymentFeeRate(currency) * 100).toLocaleString("fr-FR")} %)`}
              value={`− ${fmt(serviceFee, currency)}`}
              muted
            />
            <div className="my-2 border-t border-border/60" />
            {tooLow ? (
              <div className="rounded-md border border-red-500/30 bg-red-500/10 px-3 py-2 text-xs text-red-300">
                <span className="font-semibold">Transfert impossible :</span> montant trop faible
                pour couvrir les frais. Saisissez au moins{" "}
                {fmt(minimumInvoiceAmount(currency), currency)}.
              </div>
            ) : (
              <Row label="Vous recevez (net)" value={fmt(net, currency)} bold />
            )}
            {!tooLow && payoutCurrency !== currency && (
              <>
                <Row
                  ...
                  value={
                    loadingRate
                      ? "…"
                      : rate
                        ? `1 ${currency} = ${rate.toLocaleString("fr-FR", { maximumFractionDigits: 4 })} ${payoutCurrency}`
                        : "—"
                  }
                  muted
                />
                <Row
                  ...
                  value={
                    netLocal
                      ? `${Math.round(netLocal).toLocaleString("fr-FR")} ${payoutCurrency}`
                      : "—"
                  }
                  bold
                  highlight
      ...
      <DialogFooter>
        <Button
          disabled={!amount || !description || submitting || tooLow}
          onClick={() => onSubmit({ amount: amt, currency, description })}
        >
...
function Row({
  ...
  return (
    ...
    >
      ...
      <span className={`${bold ? "font-semibold" : ""} ${highlight ? "text-emerald-400" : ""}`}>
        {value}
      </span>
    </div>
  );
