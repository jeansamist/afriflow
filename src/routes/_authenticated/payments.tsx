...
function PaymentsPage() {
  ...
  useEffect(() => {
    ...
    if (currency === payoutCurrency) { setPreview({ rate: 1, source: "same" }); return; }
    let cancelled = false;
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAmount(""); setDescription(""); setClientId(NONE);
      void refresh();
    } catch (e: any) {
  ...
  const statusVariant = (s: string) =>
    s === "PAID" ? "bg-primary/20 text-primary border-primary/30"
      : s === "EXPIRED" || s === "CANCELLED" ? "bg-red-500/10 text-red-300 border-red-500/30"
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Encaissez vos clients à l'étranger en EUR, USD, CAD… Reçu automatique envoyé.</p>
          </div>
          ...
          <form onSubmit={handleCreate} className="grid gap-4">
            <div className="grid gap-4 md:grid-cols-[1fr_120px_140px]">
              <div>
                ...
                <Input id="desc" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Prestation, acompte..." required />
              </div>
              <div>
                ...
                <Select value={currency} onValueChange={setCurrency}>
                  ...
                  <SelectContent>
                    {INVOICE_CURRENCIES.map((c) => <SelectItem key={c} value={c}>{c}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div>
                <Label htmlFor="amount">Montant *</Label>
                <Input id="amount" type="number" step="0.01" min="0.5" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="100.00" required />
              </div>
            </div>
            ...
            <div className="rounded-lg border border-border bg-surface-elevated/40 px-4 py-4 text-sm">
              <div className="flex items-center justify-between gap-3 mb-3">
                ...
                {previewLoading && <RefreshCw className="h-3 w-3 animate-spin text-muted-foreground" />}
              </div>

              {!breakdown ? (
                <p className="text-muted-foreground text-xs">Saisissez un montant pour voir le détail (taux, frais, montant reçu).</p>
              ) : (
                <div className="space-y-2">
                  <Row label="Votre client paie" value={`${Number(amount).toFixed(2)} ${currency}`} strong />
                  ...
                  <Row label="Frais de transaction" value={`− ${breakdown.feeInvoice.toFixed(2)} ${currency}`} hint={paymentFeeLabel(currency)} />
                  ...
                  {mmNumber ? (
                    <div className="mt-2 rounded-md border border-border/60 bg-background/40 px-3 py-2 text-xs">
                      ...
                      <div className="mt-0.5 font-mono text-foreground">
                        {mmOperator ? `${mmOperator} · ` : ""}{mmNumber}
                      </div>
                    ...
                    <div className="mt-2 text-xs text-amber-300">
                      ...
                      <Link to="/billing" className="underline">Configurer maintenant</Link>
                    </div>
                  )}
                </div>
              )}
        ...
        <Card className="bg-card border-border overflow-hidden">
          <div className="p-4 border-b border-border"><h2 className="font-semibold">Historique</h2></div>
          {loading ? (
            <div className="p-8 text-center text-muted-foreground">Chargement...</div>
          ) : rows.length === 0 ? (
            <div className="p-8 text-center text-muted-foreground">Aucun lien. Créez-en un ci-dessus.</div>
          ) : (
            <>
              ...
              {rows.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE).map((r) => (
                <li key={r.id} className="p-4 flex items-center gap-4">
                  ...
                  <div className="text-right">
                    ...
                    {r.local_amount != null && r.local_currency && r.local_currency !== r.currency && (
                      <div className="text-xs text-muted-foreground">
                        ≈ {Number(r.local_amount).toLocaleString("fr-FR", { maximumFractionDigits: 2 })} {r.local_currency}
                      </div>
                    )}
                  ...
                  <div className="flex gap-2">
                    {r.status === "PAID" ? (
                      ...
                      <>
                        <Button size="sm" variant="ghost" onClick={() => copyLink(r.id)}><Copy className="w-4 h-4" /></Button>
            ...
            {rows.length > PAGE_SIZE && (
              <div className="flex items-center justify-between gap-3 border-t border-border p-4 text-xs text-muted-foreground">
                <span>Page {page} sur {Math.ceil(rows.length / PAGE_SIZE)} · {rows.length} lien(s)</span>
                <div className="flex gap-1">
                  <Button size="sm" variant="outline" disabled={page === 1} onClick={() => setPage((p) => Math.max(1, p - 1))}>Précédent</Button>
                  <Button size="sm" variant="outline" disabled={page >= Math.ceil(rows.length / PAGE_SIZE)} onClick={() => setPage((p) => p + 1)}>Suivant</Button>
                </div>
              </div>
            )}
            </>
...
function Row({ label, value, hint, strong, accent }: { label: string; value: string; hint?: string; strong?: boolean; accent?: boolean }) {
  return (
    <div className="flex items-start justify-between gap-3">
      <div className="text-muted-foreground">{label}</div>
      <div className="text-right">
        <div className={accent ? "text-base font-bold text-primary" : strong ? "font-semibold text-foreground" : "text-foreground"}>{value}</div>
